class BankHandleUtils {

  static String? getBankName(String cardNumber) {
    cardNumber = cardNumber.replaceAll(' ', '').trim();
    final List<int> bankBin = [621098, 622150, 622151, 622181, 622188, 955100, 621095, 620062, 621285, 621798, 621799,
    ...
    final List<String> bankName = ["邮储银行·绿卡通", "邮储银行·绿卡银联标准卡", "邮储银行·绿卡银联标准卡", "邮储银行·绿卡专用卡", "邮储银行·绿卡银联标准卡",
    ...
    int bin = int.parse(charBin);
    int index = bankBin.indexOf(bin);//binarySearch(bankBin, bin);
 ...
 */
  ...
  /// @param cardId
  /// @return
  static bool checkBankCard(String cardId) {
    if (cardId.length < 2) return false;

    cardId = cardId.replaceAll(' ', '').trim();

    String bit = getBankCardCheckCode(cardId.substring(0, cardId.length - 1));

    if (bit == 'N') {
      return false;
    }

    return cardId.runes.last == bit.runes.first;
  }


  static String getBankCardCheckCode(String nonCheckCodeCardId) {
    if (nonCheckCodeCardId.trim().isEmpty ||
        !RegExp(r'\d+').hasMatch(nonCheckCodeCardId)) {
      return 'N';
    }

    List<int> chs = nonCheckCodeCardId.trim().runes.toList();
    int luhmSum = 0;

    for (int i = chs.length - 1, j = 0; i >= 0; i--, j++) {
      int k = chs[i] - 48; // ASCII code for '0'
    ...
    return (luhmSum % 10 == 0) ? '0' : String.fromCharCode(10 - luhmSum % 10 + 48); // ASCII code for '0'
  }
